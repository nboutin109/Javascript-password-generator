...
function makePassword(){
  if (charSelection.lcl == "True" && charSelection.ucl == "True" && charSelection.num == "True" && charSelection.spc == "True"){var ERROR = "Please leave at least one box unchecked"; 
    $('#errors').html(ERROR);
    $('#passwordArea').html('');
  return}
  if(charSelection.length > 128 || charSelection.length < 8 || charSelection.length == isNaN){var ERROR = "Please ensure your password length is a number between 8 and 128"; 
    $('#errors').html(ERROR);
    $('#passwordArea').html('');
  return}
  else $('#errors').html('');
